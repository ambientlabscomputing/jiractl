...
def validate_epics(
    epics_df: pd.DataFrame, config: Config
) -> tuple[list[Epic], list[dict]]:
    """
...
def validate_stories(
    stories_df: pd.DataFrame, config: Config, epics: list[Epic]
) -> tuple[list[Story], list[dict]]:
    """
...
def validate(ctx: click.Context, input_dir: str):
    ...
    for error in epic_errors:
        epics_table.add_row(
            str(error["row"]), "[red]✗[/red]", f"[red]{error['error']}[/red]"
        )
