...
@click.option(
    "-s", "--status", default=None, help="Filter by status (e.g. 'In Progress')."
)
@click.option(
    "--assignee", default=None, help="Filter by assignee display name or account ID."
)
@click.option("--text", default=None, help="Full-text search within issues.")
@click.option(
    "--mine", is_flag=True, default=False, help="Show only issues assigned to you."
)
@click.option(
    "--epics-only", is_flag=True, default=False, help="List only epics in the project."
)
@click.option(
    "-n", "--limit", default=50, show_default=True, help="Max results to show."
)
@format_option()
@click.pass_context
def list_issues(
    ...
    if not project and not epic_key and not mine:
        ...
        if output_format == "json":
            ...
            print(
                json.dumps(
                    [
                        {"project": p, "aliases": cfg.synonyms.get(p, [])}
        ...
        if epic_key:
            ...
            with use_status("Fetching your issues..."):
                issues = get_my_issues(
                    client, project_key=project, status=status, max_results=limit
                )
            title = "My Issues"

        elif epics_only:
            proj = (
                (cfg.resolve_project(project) or project.upper()) if project else None
            )
            if not proj:
