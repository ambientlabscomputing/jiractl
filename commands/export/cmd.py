...
@click.option(
    "-a", "--assignee", default=None, help="Filter by assignee display name or 'me'."
)
@click.option(
    "-s",
    "--status",
...
@click.pass_context
def export(
    ctx, project, issue_type, assignee, status_filter, output_file, include_description
):
    """
    ...
    def _write(writer_dest):
        writer = csv.DictWriter(
            writer_dest, fieldnames=columns, extrasaction="ignore", lineterminator="\n"
        )
        writer.writeheader()
        for issue in issues:
    ...
    if output_file:
        with open(output_file, "w", newline="", encoding="utf-8") as f:
            _write(f)
        console.print(
            f"[green]✓[/green] Exported {len(issues)} issues to [bold]{output_file}[/bold]"
        )
    else:
        _write(sys.stdout)
