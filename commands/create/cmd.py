...
from jira_api.issues import create_epic, create_story, resolve_issue_type_name
...
def _pick_project(config, project_flag: str | None) -> str:
    ...
    if project_flag:
        ...
        if key not in config.allowed_projects:
            console.print(
                f"[yellow]Warning: '{key}' is not in allowed_projects[/yellow]"
            )
        return key
    choices = config.allowed_projects
    console.print(
        "Available projects: " + ", ".join(f"[cyan]{p}[/cyan]" for p in choices)
    )
...
@click.option("-p", "--project", default=None, help="Project key (e.g. TCRM)")
@click.option(
    "-e", "--epic", "epic_key", default=None, help="Parent epic key (e.g. TCRM-1)"
)
...
def create_ticket_cmd(
    ctx, project, epic_key, summary, description, issue_type, dry_run
):
    """
    ...
    if not epic_key:
        epic_key = (
            click.prompt(
                "Parent epic key (leave blank to skip)", default="", show_default=False
            )
            or None
        )
    ...
    parent_line = (
        f"[dim]Parent:[/dim] {epic_key}" if epic_key else "[dim]No parent epic[/dim]"
    )
    console.print(
        Panel(
            f"[bold]{summary}[/bold]\n{parent_line}\n\n{description or '[dim]No description[/dim]'}",
    ...
    url = f"{cfg.base_url}/browse/{key}"
    console.print(
        f"[green]✓[/green] Created {issue_type} [bold cyan]{key}[/bold cyan]  {url}"
    )
