...
from jira_api.search import adf_to_text
...
_COLS_WITH_PROJECT = ("key", "project", "type", "summary", "status", "priority", "assignee", "updated")
...
def _print_tsv_detail(issue: dict) -> None:
    cols = ("key", "project", "type", "summary", "status", "priority",
            "assignee", "reporter", "parent", "labels", "created", "updated")
    flat = flatten_issue(issue, include_description=True)
    print("\t".join(c.upper() for c in cols))
    print("\t".join(flat.get(c, "") for c in cols))
    ...
    *,
    ...
    if fmt == "json":
        ...
        if children is not None:
            ...
            console.print(
                f"\n[bold]Child Issues[/bold] of [cyan]{parent_key}[/cyan]"
                f" ({len(children)} found)\n"
            )
            if children:
