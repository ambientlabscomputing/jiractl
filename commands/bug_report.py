...
from commands._version import __version__
from commands.ui import console
...
def _redact(text: str, email: Optional[str] = None, token: Optional[str] = None) -> str:
...
def _run_reporter(
    ...
    safe_traceback = (
        _redact(traceback_text, email=email, token=token) if traceback_text else None
    )
...
class BugReportGroup(click.Group):
    ...
    def add_command(self, cmd: click.BaseCommand, name: Optional[str] = None) -> None:
