...
from jiractl.jira_api.search import (
...
def inspect_project(ctx, project_key, output_format):
    ...
    console.print(Panel(info_table, title=f"[cyan]{project_key}[/cyan] Project", border_style="cyan"))
...
def inspect_fields(ctx, project_key, issue_type_name, custom_only, output_format):
    ...
    with make_client(ctx) as client:
        ...
        if not issue_types:
            raise click.ClickException(f"No issue types found for project '{project_key}'.")
        ...
        if issue_type_name:
            ...
            target = next((it for it in issue_types if not it.get("subtask")), issue_types[0])
    ...
    if custom_only:
        fields = [
            f for f in fields if f.get("schema", {}).get("custom") or f.get("fieldId", "").startswith("customfield_")
        ]
    ...
    for f in sorted(fields, key=lambda x: (not x.get("required", False), x.get("name", "").lower())):
        fid = f.get("fieldId") or f.get("key", "")
        name = f.get("name", "")
        schema = f.get("schema", {})
