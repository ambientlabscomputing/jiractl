...
from jiractl.commands.ui import console, format_option, status_style
from jiractl.jira_api.mutations import (
    find_transition,
    get_transitions,
    transition_issue,
)
from jiractl.jira_api.search import get_issue
    ...
    with make_client(ctx) as client:
        ...
        if list_transitions:
            ...
            if output_format == "json":
                    ...
                    json.dumps(
                        {
                            ...
                            "transitions": [{"id": t["id"], "name": t["to"]["name"]} for t in transitions],
                        },
                        indent=2,
                    )
                )
                ...
                console.print(f"\nCurrent status of [cyan]{issue_key}[/cyan]: [{ss}]{current}[/{ss}]\n")
                table = Table(
        ...
        if output_format == "json":
            ...
            console.print(f"[green]✓[/green] [cyan]{issue_key}[/cyan] → [{ss}]{target_status}[/{ss}]")
