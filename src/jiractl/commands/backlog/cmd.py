    ...
    with make_client(ctx) as client:
        if sprint_ref:
            ...
            if sprint is None:
                ...
                if is_kanban_board(client, board_id):
                    msg = (
                        f"Board {board_id} is a Kanban board. The 'backlog sprint' "
                        "command only works with Scrum boards that have sprints."
                    )
                    if output_format == "json":
                        print("[]")
...
def backlog_sprints(ctx, project_ref, board_id, state, output_format):
    ...
    # Check if this is a Kanban board
    if not sprints and is_kanban_board(client, board_id):
        msg = f"Board {board_id} is a Kanban board. The 'backlog sprints' command only works with Scrum boards."
        if output_format == "json":
            print("[]")
        elif output_format == "bash":
            print(f"# {msg}")
...
@click.option(
    ...
    "sprint_ref",
    default=None,
    help="Sprint name or numeric ID. Defaults to the active sprint (Scrum boards only).",
)
@click.pass_context
def backlog_add(ctx, issue_keys, project_ref, board_id, sprint_ref):
    """
    Move one or more issues from the backlog to the board.

    For Scrum boards: moves issues into a sprint.
    For Kanban boards: moves issues directly to the board (no sprint).

    The project is derived from the issue key prefix (e.g. TCRM-1 → TCRM)
    unless overridden with -p/--project.
    ...
    with make_client(ctx) as client:
        ...
        if is_kanban_board(client, board_id):
            if sprint_ref:
                raise click.ClickException(
                    f"Board {board_id} is a Kanban board. "
                    "Kanban boards don't have sprints, so --sprint is not applicable."
                )
            # For Kanban: move directly to the board
            with console.status(f"Moving {len(keys)} issue(s) to Kanban board..."):
                move_to_kanban_board(client, board_id, keys)
            n = len(keys)
            console.print(f"[green]✓[/green] Moved {n} issue{'s' if n != 1 else ''} to the board")
        else:
            # For Scrum: find sprint and move
            if sprint_ref:
                ...
                if sprint is None:
                    raise click.ClickException(
                        "No active sprint found. Use --sprint NAME_OR_ID to specify one explicitly."
                    )
            ...
            n = len(keys)
            console.print(
                f"[green]✓[/green] Moved {n} issue{'s' if n != 1 else ''} to sprint [bold]{sprint_name}[/bold]"
            )
