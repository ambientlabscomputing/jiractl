...
from jiractl.commands.batch.transform.cmd import run_transform
from jiractl.commands.batch.validate.cmd import validate_epics, validate_stories
from jiractl.jira_api.issues import create_epic, create_story
...
def upload(
    ...
    try:
        ...
        if epic_errors or story_errors:
            console.print(f"[red]Validation failed: {len(epic_errors) + len(story_errors)} errors[/red]")
            ctx.exit(1)
            ...
            with Progress(
                ...
                for epic in valid_epics:
                    ...
                    if epic.epic_name in state["epics"]:
                        ...
                        console.print(
                            f"[dim]↩ Skipping epic '{epic.epic_name}' (already {state['epics'][epic.epic_name]})[/dim]"
                        )
                        continue
                    try:
            ...
            with Progress(
                ...
                for story in valid_stories:
                    ...
                    if sk in state["stories"]:
                        ...
                        console.print(f"[dim]↩ Skipping story '{story.summary}' (already {state['stories'][sk]})[/dim]")
                        continue
