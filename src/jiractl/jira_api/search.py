...
def search_all(
    ...
    while True:
        page, cursor = search_issues(client, jql, fields=fields, max_results=page_size, next_page_token=cursor)
        issues.extend(page)
...
def get_epic_children(client: JiraClient, epic_key: str, max_results: int = 100) -> list[dict]:
    """
...
def get_project_epics(client: JiraClient, project_key: str, max_results: int = 100) -> list[dict]:
